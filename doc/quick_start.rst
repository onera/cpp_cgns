...
Below is a succinct presentation of the main facilities of **cpp_cgns**. For a more detailed presentation, see the :ref:`user_manual`.

Everything in **cpp_cgns** is defined within the :cpp:`cgns` namespace.
...
The interest of this strategy comes from the fact that it is not exclusive: there can be several allocators associated to several life durations of nodes of the tree; and it can be combined with other strategy, e.g. the external allocation of Python/CGNS tree if the tree is indeed also used on the Python side. Memory management is detailed :ref:`here <memory_management>`.
...
C++/CGNS trees are compatible with python/CGNS trees. Functions :cpp:`view_as_pytree` and :cpp:`view_as_cpptree` from file ``interop/pycgns_converter.hpp`` can be called on the C++ side to convert from/to python/CGNS trees.
...
The returned PyObject is a regular python/CGNS tree. Its numpy arrays do not allocate memory, they just point to the same memory as the :cpp:`cgns::tree`.
...
The memory is shared in both cases. If the user wants a separate copy of the tree, the :cpp:`deep_copy` function can be used.

C and Fortran [EXPERIMENTAL]
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:cpp:`cgns::tree` objects can be handled in C via the :cpp:`cgns_tree` opaque structure and its access functions in file ``interop/c_interop.hpp``.
