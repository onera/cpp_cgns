...
If you know how the library works but are looking for a list of functionnalities, check out the :ref:`api`.

You can also check out the installation procedure here: :ref:`installation`.

Everything is defined within the :cpp:`cgns` namespace. When there is no ambiguity, we will omit the :cpp:`cgns::` namespace.
...
The :cpp:`node_value` interface is flexible and can be used for small arrays. Moreover, having one single type for a node value is mandatory to built a proper tree structure. However, for efficiency reasons, and also because it is often nice to use static typing, a :cpp:`node_value` can be viewed as a "regular" multi-dimensional array with fixed type and fixed rank.
...
See the complete list of functions here: :ref:`tree_manip_api`.
...
The :cpp:`gen_path` argument of the two last functions is a "generalized" path: a "/"-separated string, with each element being the name of a node (like a regular path) or the label of a node. Example:
...
Direct access to typed node value
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If you want to get the array value of a node directly, the following functions chain the search for the node with the creation of a typed multi-diremensional view of the array:

.. code:: c++

  template<class T, int rank> auto get_value                 (tree& t);
  template<class T, int rank> auto get_child_value_by_name   (tree& t, const std::string& s);
  template<class T, int rank> auto get_child_value_by_label  (tree& t, const std::string& s);
  template<class T, int rank> auto get_node_value_by_matching(tree& t, const std::string& s);

Example:

.. code:: c++

  auto pe = get_node_value_by_matching<I4,2>(zone_node,"MyQuads/ParentElements");
  int first_left_parent = pe(0,0);

The :cpp:`rank` template parameter defaults to :cpp:`1` and in this case the returned view is a :cpp:`span`.

Node creation
-------------

There are many convenience functions to create CGNS nodes of different labels. See the complete list here: :ref:`node_creation_api`.

Example:

.. code:: c++

  tree pl = new_PointList("PointList", std::vector<I4>{1,10,5,4});
...
A typical workflow is to have a CGNS tree on the Python side, on which we want to apply a performance-sensitive algorithm in C++. For that, we create a C++/CGNS tree from the Python/CGNS tree, apply the algorithm to it, and them update the Python/CGNS tree to match the changes.

The two functions to use are:

.. code:: c++

  auto to_cpp_tree(py::list pytree) -> tree;
  auto update_py_tree(tree&& t, py::list pytree) -> void;

The :cpp:`py_tree` argument is a regular Python/CGNS tree, accessible in C++ through the Pybind11 :cpp:`py::list` type. The arrays in the resulting C++/CGNS tree are just views pointing to the Python/CGNS arrays and the memory is valid as long as the Python tree is alive.

In :cpp:`update_py_tree`, the memory ownership of the C++/CGNS arrays is transfered to Python through the capsule mechanism (Note: this ownership transfer is a cheap operation since only the ownership is transfered, the memory itself does not need to be moved or copied. Concretely, this mean that the Python garbage collector will deallocate the memory allocated from the C++ side).

Note that the C++/CGNS tree is passed by rvalue reference: it is not meant to be reused afterwards (Note: as of today, it technically could, but this is an implementation detail that could change in the future).

We update the tree rather than creating one from scratch because it ensures that the :cpp:`py_tree` argument is modified in-place, not rebound to a new object. So Python objects referencing :cpp:`pytree` before applying :cpp:`to_cpp_tree | algorithm | update_py_tree` will remain valid and reference the modified :cpp:`pytree`.

However, we may want to create a Python tree from scratch with this function:

.. code:: c++

  auto to_py_tree(tree&& t) -> py::list;

Here again, only ownership transfer of arrays is performed, and the complexity of the whole operation is proportional to the number of nodes in the tree, **not** to the size of the data.
