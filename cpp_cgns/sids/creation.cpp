...
/// node creation {
tree factory::newDataArray(const std::string& name, node_value value) {
  return {name, value, {}, "DataArray_t"};
}
tree factory::newUserDefinedData(const std::string& name, node_value value) {
  return {name, value, {}, "UserDefinedData_t"};
}
tree factory::newUserDefinedData(const std::string& name, const std::string& val) {
  node_value value = create_string_node_value(val,alloc());
  return {name, value, {}, "UserDefinedData_t"};
}

tree factory::newRootNode() {
  return {"CGNSTree", MT, {}, "CGNSTree_t"};
}

tree factory::newCGNSVersionNode() {
  R4 version = 3.1;
  node_value version_arr = create_node_value_1d({version},alloc());
  return {"CGNSLibraryVersion", version_arr, {}, "CGNSLibraryVersion_t"};
}

tree factory::newCGNSTree() {
  tree t = newRootNode();
  t.children.push_back(newCGNSVersionNode());
  return t;
}

tree factory::newCGNSBase(const std::string& name, I4 cellDim, I4 physDim) {
...
tree factory::newUnstructuredZone(const std::string& name, const std::array<I4,3>& dims) {
...
tree factory::newZoneBC() {
  return {std::string("ZoneBC"), MT, {}, "ZoneBC_t"};
}
tree factory::newZoneGridConnectivity() {
  return {std::string("ZoneGridConnectivity"), MT, {}, "ZoneGridConnectivity_t"};
}

tree factory::newGridCoordinates(const std::string& name) {
  return {name, MT, {}, "GridCoordinates_t"};
}


tree factory::newPointRange(I4 first, I4 last) {
...
tree factory::newElementRange(I4 first, I4 last) {
  node_value range_value = create_node_value_1d({first,last},alloc());
...
tree factory::newPointList(const std::string& name, std_e::span<I4> pl) {
...
tree factory::newBC(const std::string& name, const std::string& loc, std_e::span<I4> point_list) {
...
tree factory::newFlowSolution(const std::string& name, const std::string& gridLoc) {
  tree loc = newGridLocation(gridLoc);
  return {name, MT, {loc}, "FlowSolution_t"};
}
tree factory::newRind(const std::vector<I4>& rind_planes) {
  node_value value = create_node_value_1d(rind_planes,alloc());
  return {"Rind", value, {}, "Rind_t"};
}
tree factory::newDiscreteData(const std::string& name, const std::string& gridLoc) {
  tree loc = newGridLocation(gridLoc);
  return {name, MT, {loc}, "DiscreteData_t"};
}
tree factory::newZoneSubRegion(const std::string& name, int dim, const std::string& gridLoc) {
  node_value dim_value = create_node_value_1d({dim},alloc());
  tree loc = newGridLocation(gridLoc);
  return {name, dim_value, {loc}, "ZoneSubRegion_t"};
}
tree factory::newBCDataSet(const std::string& name, const std::string& val, const std::string& gridLoc) {
  tree loc = newGridLocation(gridLoc);
  node_value value = create_string_node_value(val,alloc());
  return {name, value, {loc}, "BCDataSet_t"};
}
tree factory::newBCData(const std::string& name) {
  return {name, MT, {}, "BCData_t"};
}



tree factory::newGridLocation(const std::string& loc) {
  node_value location = create_string_node_value(loc,alloc());
  return {"GridLocation",location,{},"GridLocation_t"};
}
tree factory::newFamily(const std::string& name) {
  return {name, MT, {}, "Family_t"};
}
tree factory::newFamilyBC(const std::string& famName) {
  node_value famNameValue = create_string_node_value(famName,alloc());
  return {"FamilyBC", famNameValue, {}, "FamilyBC_t"};
}

tree factory::newDescriptor(const std::string& name, const std::string& val) {
  node_value value = create_string_node_value(val,alloc());
  return {name, value, {}, "Descriptor_t"};
}

tree factory::newGridConnectivityType(const std::string& gc_type) {
...
tree factory::newGridConnectivity(const std::string& name, const std::string& z_donor_name, const std::string& loc, const std::string& connec_type) {
...
/// node removal {
void factory::deallocate_node(tree& t) {
  alloc().deallocate(t.value.data);
}
void factory::deallocate_tree(tree& t) {
  deallocate_node(t);
...
void factory::rm_child_by_predicate(tree& t, Unary_predicate p) {
  auto& cs = t.children;
...
template<class Unary_predicate>
void factory::rm_children_by_predicate(tree& t, Unary_predicate p) {
  auto& cs = t.children;
...
void factory::rm_child_by_name(tree& t, const std::string& name) {
